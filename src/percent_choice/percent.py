...
class Percent:
    ...
    def __init__(self, objects, chances):
        if len(objects) != len(chances):
            raise ValueError("objects and chances must have the same length")
        if not objects:
            raise ValueError("objects cannot be empty")
        if not all(isinstance(x, (int, float)) for x in chances):
            raise TypeError("all values in chances must be int or float")
        if any(x < 0 for x in chances):
            raise ValueError("chances cannot be negative")
        if sum(chances) != 100:
            raise ValueError("sum of chances must be 100")
